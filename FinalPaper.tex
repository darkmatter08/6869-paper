...
Specialized datasets were necissary for the subnetwork identification visualization technique described in section \ref{sec:subnetwork}.
...
\caption{Generated text with constraints applied.}
\label{fig:genText}
\end{figure}
...
||Rough needs work and elaboration||
We propose a method to isolate subnetworks by using activations. Similar to how a neurologist would map the brain, we probe the network with different datasets and observe how the network reacts. In particular our method focuses on the amount of activation at each node.
||Elaborate on method||

This method attempts to visualize the structure of the network rather than node specific operations as is typically done. The structure view provides and abstraction that is usefull with discussing operations that are difficult or unnecissary to visualize. For example, it is difficult and uninformative to visualize transformations (rotation, scale, translation, etc). However understanding which parts of the network are active durring different transformations can be informative.
||Discribe possibile uses of this informations (here or somewhere else?)||
